...
export class ListAssignMediaComponent implements OnInit {
  ...
  constructor(
    ...
    if (!this.nivId || !this.nivMatId || !this.chapId || !this.mediaTypeId)
      this._location.back();
  ...
  getAssignMediaRating(id) {
    this.mediaReviewService.getAssignMediaRating(id).subscribe(
      (res) => {
  ...
  mediaType: MediaType;
  getMediaTypeById() {
    this.mediaTypeService.findById(this.mediaTypeId).subscribe(
      (res: MediaType) => {
        console.log('res : ', res);
        if (res) {
          this.mediaType = res;
  ...
  getChapitreMedias() {
    this.mediaAssignService
      .findByAssignId(this.chapId, this.mediaTypeId)
      .subscribe(
        (res: MediaAssign[]) => {
  ...
  onSubmit() {
    if (this.form.value) {
      ...
      if (this.modalType === 'add') {
        ...
        this.mediaAssignService.create(body).subscribe((data: any) => {
