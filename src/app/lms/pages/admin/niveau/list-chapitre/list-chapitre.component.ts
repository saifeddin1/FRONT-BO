...
import {
  NivMatChapitre,
  NivMatChapitreService,
  GetAllResult,
} from '../nivMatChapitre.service';
...
export class ListChapitreComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (
      (user && user.type === INSTRUCTOR && user.permissions.chapitre) ||
      user.type === ADMIN
    )
      this.canManage = true;
  }
  ...
  chapitreDataSource: MatTableDataSource<Chapitre> =
    new MatTableDataSource<Chapitre>();
  displayedSearchChapitreColumns: string[] = ['name', 'action'];
  getAllChapitreQueriable(page = 0, limit = 10, text: string = '') {
    this.chapitreService.search(page, limit, text).subscribe(
      (result: { totalCount: { count: number }; totalData: [Chapitre] }) => {
  ...
  getAssignedChapitresByNivMatId() {
    this.nivMatChapitreService.getAll(this.nivId, this.nivMatId).subscribe(
      (res: GetAllResult) => {
        console.log('res : ', res);
        if (res) {
          this.data = res;
          console.log('this.data :', this.data);
          this.nivMatChapitres = new MatTableDataSource<any>(
            res.nivMatChaps.sort((a, b) => a.order - b.order)
          );
  ...
  drop(event: CdkDragDrop<NivMatChapitre[]>) {
    const previousIndex = this.nivMatChapitres.data.findIndex(
      (row) => row === event.item.data
    );
    moveItemInArray(
      this.nivMatChapitres.data,
      previousIndex,
      event.currentIndex
    );
    this.nivMatChapitres.data = this.nivMatChapitres.data.map(
      (item: Chapitre, index: number) => ({ ...item, order: index })
    );
    const chapitres = {
      data: this.nivMatChapitres.data.map((x) => ({
        _id: x._id,
        order: x.order,
      })),
    };
    this.nivMatChapitreService.editOrders(chapitres).subscribe(
      (res) => {
  ...
  onSubmit() {
    if (this.chapitreMode == 'add') {
      ...
      this.nivMatChapitreService
        ...
        .subscribe(
          (res) => {
            ...
            this.nivMatChapitres = new MatTableDataSource(
              this.nivMatChapitres.data
            );
      ...
      this.nivMatChapitreService
        .editByIdwithChapitre(this.form.value._id, this.form.value)
        .subscribe(
          (res: any) => {
            var array = this.nivMatChapitres.data;
            var foundIndex = array.findIndex(
              (el: Chapitre) => el._id == res._id
            );
            if (foundIndex > -1) {
  ...
  deleteNivMatChapitreId(nivMatChapId, index) {
    ...
    this.nivMatChapitreService.delete(nivMatChapId).subscribe(
      (res) => {
  ...
  deleteChapitreById(_id, index): void {
    this.chapitreService.delete(_id).subscribe(
      (res) => {
        ...
        this.chapitreDataSource = new MatTableDataSource(
          this.chapitreDataSource.data
        );
  ...
  assignChapire(element) {
    ...
    this.nivMatChapitreService.create(body).subscribe(
      (res) => {
