...
export class ListCoursComponent {
  ...
  displayedCourseColumns: string[] = ['name', 'enabled', 'action'];
  ...
  getCourse() {
    this.courseService
      .getAllByChapId(this.nivId, this.nivMatId, this.chapId)
      .subscribe(
        (res: CourseGetAllResult) => {
          console.log('res : ', res);
          if (res) {
            this.data = res;
            console.log('this.data :', this.data);

            this.courses = new MatTableDataSource<Course>(
              res.courses.sort((a, b) => a.order - b.order)
            );
  ...
  dropCourse(event: CdkDragDrop<Course[]>) {
    const previousIndex = this.courses.data.findIndex(
      (row) => row === event.item.data
    );
    moveItemInArray(this.courses.data, previousIndex, event.currentIndex);
    this.courses.data = this.courses.data.map(
      (item: Course, index: number) => ({ ...item, order: index })
    );
    const courses = {
      data: this.courses.data.map((x) => ({ _id: x._id, order: x.order })),
    };
    this.courseService.editOrders(courses).subscribe(
      (res) => {
