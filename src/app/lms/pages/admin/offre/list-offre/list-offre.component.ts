...
export class ListOffreComponent implements OnInit {
        ...
        if (res && res.length) {
          res.forEach((offre) => {
            if (offre.img) {
              debugger;
              offre.img =
                offre.img === '' || offre.img === null
                  ? ''
                  : `${environment.LmsApiUrl}/api/offre/documents/${offre.img}`; // TODO REMOVE LOCALHOST FROM PRODUCTION BUILD
            }
          });
        }
    ...
    for (const droppedFile of this.img) {
      if (droppedFile.fileEntry.isFile) {
        ...
        fileEntry.file((file: File) => {
          ...
          this.imageService
            .uploadImage(formData, this.form.value._id, 'offre')
            .subscribe(
              (res) => {
  ...
  onSubmit() {
    if (this.form.value) {
      ...
      if (this.modalType === 'add') {
        this.offreService.create(body).subscribe(
          (data: any) => {
        ...
        //edit modal service
        this.offreService.editById(this.form.value._id, body).subscribe(
          (data: Offre) => {
              ...
              (res) => {
                ...
                if (res.img) {
                  this.dataSource.data[
                    foundIndex
                  ].img = `${environment.LmsApiUrl}/api/offre/documents/${res.img}`; // TODO REMOVE LOCALHOST FROM PRODUCTION BUILD
                }
                this.dataSource = new MatTableDataSource(this.dataSource.data);
                this.img = [];
              },
  ...
  deleteById(_id, index) {
    this.offreService.delete(_id).subscribe(
      (res: any) => {
        console.log('res :', res);
