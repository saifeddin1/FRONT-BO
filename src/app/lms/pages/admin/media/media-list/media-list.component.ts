...
export class MediaListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (
      (user && user.type === INSTRUCTOR && user.permissions.media) ||
      user.type === ADMIN
    )
      this.canManage = true;
  }
    ...
    for (const droppedFile of this.img) {
      if (droppedFile.fileEntry.isFile) {
        ...
        fileEntry.file((file: File) => {
          ...
          this.imageService
            .uploadImage(formData, this.form.value._id, 'media')
            .subscribe(
              (res) => {
  ...
  onSubmit() {
    if (this.form.value) {
      ...
      var body: Partial<Media> = {
        ...
        type:
          typeof this.form.value.type == 'object'
            ? this.form.value.type._id
            : this.form.value.type,
      };
      if (this.modalType === 'add') {
        if (this.img) {
          this.mediaService.create(body).subscribe(
            (data: any) => {
        ...
        //edit modal service
        this.mediaService.editById(this.form.value._id, body).subscribe(
          (data: Media) => {
  ...
  fillFormModel(body) {
    this.form.patchValue({
      _id: body._id,
      type:
        body.type && typeof body.type == 'object' && body.type._id
          ? body.type._id
          : body.type,
      name: body.name,
    });
  ...
  deleteById(_id, index) {
    this.mediaService.delete(_id).subscribe(
      (res: any) => {
        console.log('res :', res);
