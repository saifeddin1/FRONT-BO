...
import {
  MediaType,
  MediaTypeService,
} from '../../../../pages/admin/media/media-type.service';
import {
  MediaAssign,
  MediaAssignService,
} from '../../../../pages/admin/niveau/list-assign-media/mediaAssign.service';
import { MediaReviewService } from '../../../../services/mediaReview.service';
...
export class ChaptersComponent implements OnInit {
  ...
  confirmRating(type) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        Swal.fire('Merci!', 'Avis soumis avec succès!', 'success');
      }
    });
  }
  ...
  getPublicMediaTypes() {
    this.mediaTypeService.getPublicMediaTypes().subscribe(
      (res: MediaType[]) => {
        console.log('MediaType : ', res);
        this.mediaTypes = res.map((x) => ({
          _id: x._id,
          name: x.name,
          nameSingulier: x.nameSingulier,
        }));
      },
      (error) => {
  ...
  getChapitreMedias() {
    this.mediaAssignService
      .findByAssignIdForStudent(
        this.selectedChapitre._id,
        this.selectedMediaType._id,
        true
      )
      .subscribe(
        (res: MediaAssign[]) => {
  ...
  getRecordMedias() {
    this.mediaAssignService
      .findByAssignIdForStudent(this.nivMatId, null)
      .subscribe(
        (res: MediaAssign[]) => {
  ...
  selectedMediaType: { _id: string; name: string; nameSingulier: string } =
    null;
  ...
  assignMediaFiles: MatTableDataSource<CourseFile> =
    new MatTableDataSource<CourseFile>();
  ...
  getUserReview(type: string) {
    ...
    this.mediaReviewService.findByMediaReviewId(selectedAssignVar).subscribe(
      (res) => {
  ...
  createUserReview(comment, type: string) {
    ...
    this.mediaReviewService.create(body).subscribe(
      (res) => {
        console.log('UserReview Res :', res);
        if (res) {
          this.rating = {
      ...
      (error) => {
        console.error('error : ', error);
        const Swal = require('sweetalert2');
        Swal.showValidationMessage(`Quelque chose s'est mal passé !`);
      }
    );
  }
