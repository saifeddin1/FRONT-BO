...
export class MatiereDetailsComponent implements OnInit {
  ...
  getChapitresByNivMatId() {
    ...
    this.nivMatChapitreService.getChapitresByNivMatId(this.userService.user.studentNiveauId, this.nivMatId)
      .subscribe((nivMat: any) => {
        if (nivMat) {
          nivMat.chapitres = nivMat.chapitres.sort((a, b) => a.order - b.order)
          this.nivMat = nivMat
