...
export class StudentCalendarComponent implements OnChanges, OnInit {
  ...
  getAll() {
    ...
    this.seanceService.getAllByNivMat(viewDateStr, this.view)
      .subscribe(res => {
        console.log("res :", res);
        if (res && res.length) {
  ...
  handleEvent(event: CalendarEvent, DialogContent): void {
    ...
    this.assignMedias = new MatTableDataSource<MediaAssign>(this.modalData.meta.mediaAssigns);
  ...
  refreshView(): void { this.refresh.next(); }
