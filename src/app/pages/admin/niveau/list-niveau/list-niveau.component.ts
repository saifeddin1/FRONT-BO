...
export class ListNiveauComponent implements OnInit {
  ...
  drop(event: CdkDragDrop<Niveau[]>) {
    ...
    this.dataSource.data = this.dataSource.data.map((item: Niveau, index: number) => ({ ...item, order: index }));
    const nivMats = { nivs: this.dataSource.data.map(x => ({ _id: x._id, order: x.order })) }
    this.niveauService.editNivsOrders(nivMats)
      .subscribe((res) => {
  ...
  onSubmit() {
    if (this.form.value) {
      ...
      if (this.modalType === 'add') {
        this.niveauService.createNiveau({ ...body, order: this.dataSource.data.length })
          .subscribe(
            (data: any) => {
        ...
        //edit modal service
        this.niveauService.editById(this.form.value._id, body)
          .subscribe(
            (data: Niveau) => {
              var array: Niveau[] = this.dataSource.data;
              var foundIndex = array.findIndex((el: Niveau) => el._id == data._id);
              if (foundIndex > -1) {
