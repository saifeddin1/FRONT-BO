...
import { NiveauMatiere, NiveauService, NiveauWithMatiere } from '../niveau.service';
...
export class ListNiveauMatiereComponent implements OnInit {
  ...
  createForm() {
    const mat = this.matieresListAvailable.length ? this.matieresListAvailable[0]._id : ''
  ...
  getAllInstructorsNames() {
    this.userService.getAllInstructorsNames()
      .subscribe((res) => {
        console.log("Users : ", res);
        if (res && res.length) {
          this.instructors = res
          this.instructorsNames = res.reduce((a, v) => ({ ...a, [v._id]: v.name }), {})
        }
      }, error => {
  ...
  getInstuctorName = (instructorId) => instructorId ? this.instructorsNames[instructorId] : "Pas d'instructeur";
  ...
  matieresListAvailable: Array<Partial<Matiere>> = new Array<Partial<Matiere>>();
  matieres: Matiere[] = new Array<Matiere>();
  getAllMatiere() {
    this.matiereService.getAll()
      .subscribe((res: [Matiere]) => {
  ...
  updateAvailableMatieres() {
    ...
    this.matieresListAvailable = this.matieres.filter(mat => !matiereIds.includes(mat._id)).map(x => ({ _id: x._id, name: x.name }));
  }
  ...
  drop(event: CdkDragDrop<NiveauMatiere[]>) {
    ...
    this.dataSource.data = this.dataSource.data.map((item: NiveauMatiere, index: number) => ({ ...item, order: index }));
    const nivMats = { nivMats: this.dataSource.data.map(x => ({ _id: x._id, order: x.order })) }
    this.niveauService.editMatieresOrders(nivMats)
      .subscribe((res) => {
  ...
  dataSource: MatTableDataSource<NiveauMatiere> = new MatTableDataSource<NiveauMatiere>();
  getNiveauWithMatiere() {
    this.niveauService.findByIdwithMatieres(this.nivId)
      .subscribe((res: NiveauWithMatiere) => {
        console.log("Niveaux With Matieres : ", res);
        this.niveau = res;
        if (res.nivmats)
          this.dataSource = new MatTableDataSource(res.nivmats.filter(x => x && (typeof x == "object" && x.name)));
  ...
  onSubmit() {
    ...
    if (this.form.value) {
      if (this.modalType === 'add') {
        this.niveauService.createNivMat({ ...this.form.value, order: this.dataSource.data.length })
          .subscribe(
            (data: any) => {
              console.log('true ', data);
              this.matieresListAvailable = this.matieresListAvailable.filter(x => x._id != data.matiere)
        ...
        //edit modal service
        this.niveauService.editNivMatById(this.form.value._id, {
          userId: this.form.value.userId,
          enabled: this.form.value.enabled,
          locked: this.form.value.locked
        }).subscribe((data: any) => {
          var array: NiveauMatiere[] = this.dataSource.data;
          var foundIndex = array.findIndex((el: NiveauMatiere) => el._id == data._id);
          if (foundIndex > -1) {
  ...
  goToChapitre(nivMatId: string) {
    this.router.navigate(['niveau/matiere/chapitres/list/', this.nivId, nivMatId]);
  }

  goToEnregistrement(nivMatId: string) {
    this.router.navigate(['niveau/matiere/enregistrements/list/', this.nivId, nivMatId]);
  }
  ...
  // DELETE
  deleteById(nivMat) {
    this.niveauService.deleteNivMat(nivMat._id)
      .subscribe(
        (res: any) => {
          console.log('res :', res);
          var foundIndex = this.dataSource.data.findIndex((el) => el._id == nivMat._id);
          if (foundIndex > -1) {
