...
import { Niveau, NiveauMatiere, NiveauService } from '../../niveau/niveau.service';
import { MatPaginator } from '@angular/material/paginator';
import { InstructorNivMat, InstructorNivMatService } from './instructor-nivmat.service';
...
export class EditInstructorNivmatComponent {
  ...
  niveau: Niveau
  getNiv() {
    this.niveauService.findById(this.nivId)
  ...
  nivmatDataSource: MatTableDataSource<NiveauMatiere> = new MatTableDataSource<NiveauMatiere>();
  displayedNivMatColumns: string[] = ['name', 'action'];
  getMatieres() {
    this.niveauService.getAllMatieresById(this.nivId)
      .subscribe((res: [NiveauMatiere] | any) => {
  ...
  assignNiveau(nivMat: NiveauMatiere) {
    this.instructorNivMatService.create({ userId: this.userId, niveauId: this.nivId, nivMatId: nivMat._id })
      .subscribe((res: InstructorNivMat) => {
  ...
  dataSource: MatTableDataSource<InstructorNivMat> = new MatTableDataSource<InstructorNivMat>();
  displayedColumns: string[] = ['name', 'action'];
  getInstructorNivMats() {
    this.instructorNivMatService.byUserIdNivId(this.userId, this.nivId)
      .subscribe((res: InstructorNivMat[]) => {
        console.log("res : ", res)
  ...
  openModal() { this.clrModalOpen = true }
  closeModal() { this.clrModalOpen = false }
}
