...
export class EditInstructorNiveauComponent {
  ...
  assignNiveau(niveau: Niveau) {
    this.instructorNivService.create({ userId: this.userId, niveauId: niveau._id })
      .subscribe((res: InstructorNiv) => {
  ...
  dataSource: MatTableDataSource<InstructorNiv> = new MatTableDataSource<InstructorNiv>();
  displayedColumns: string[] = ['name', 'action'];
  getInstructorNiveaux() {
    this.instructorNivService.findByUserId(this.userId)
      .subscribe((res: InstructorNiv[]) => {
        console.log("res : ", res)
