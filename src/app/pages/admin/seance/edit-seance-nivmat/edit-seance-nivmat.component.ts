...
export class EditSeanceNivmatComponent implements OnInit {
  ...
  seance: Seance
  getSeance() {
    this.seanceService.findById(this.seanceId)
      .subscribe((res: Seance) => {
  ...
  assignNiveau(nivMat: NiveauMatiere) {
    this.seanceNivMatService.create({ seanceId: this.seanceId, niveauId: this.nivId, nivMatId: nivMat._id })
      .subscribe((res: SeanceNivMat) => {
  ...
  dataSource: MatTableDataSource<SeanceNivMat> = new MatTableDataSource<SeanceNivMat>();
  displayedColumns: string[] = ['name', 'action'];
  getSeanceNivMats() {
    this.seanceNivMatService.bySeanceIdNivId(this.seanceId, this.nivId)
      .subscribe((res: SeanceNivMat[]) => {
        console.log("res : ", res)
