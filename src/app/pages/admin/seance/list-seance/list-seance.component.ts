...
import { PaginatedResult, PaginationArgs } from 'src/app/services/pagination.service';
...
export class ListSeanceComponent implements OnInit {
  ...
  constructor(
    ...
    this.getSeances(new PaginationArgs(this.paginatorOptions.currentPage, this.paginatorOptions.pageSize));
  ...
  getNext(event) {
    ...
    const query = new PaginationArgs(event.pageIndex, event.pageSize, this.getNextSearchText)
    this.getSeances(query);
  }

  dataSource: MatTableDataSource<Seance> = new MatTableDataSource<Seance>();
  @ViewChild(MatPaginator, { static: true }) paginator: MatPaginator;
  displayedColumns: string[] = ['name', 'userId', 'url', 'startDate', 'endDate', 'action'];
  getSeances(query: PaginationArgs = new PaginationArgs()) {
    console.log("query :", query);
    this.seanceService.getAll(query)
      .subscribe((result: PaginatedResult | any) => {
  ...
  onSubmit() {
    if (this.form.value) {
      ...
      if (this.modalType === 'add') {
        this.seanceService.create(body)
          .subscribe(
            (data: any) => {
              ...
              console.log(err);
              debugger
              if (err && err.status == 400 && err.error.message && err.error.errors && Object.keys(err.error.errors).length) {
        ...
        //edit modal service
        this.seanceService.editById(this.form.value._id, body)
          .subscribe(
            (data: Seance) => {
              var array: Seance[] = this.dataSource.data;
              var foundIndex = array.findIndex((el: Seance) => el._id == data._id);
              if (foundIndex > -1) {
