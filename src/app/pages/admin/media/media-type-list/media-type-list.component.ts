...
export class MediaTypeListComponent implements OnInit {
  ...
  drop(event: CdkDragDrop<MediaType[]>) {
    ...
    this.dataSource.data = this.dataSource.data.map((item: MediaType, index: number) => ({ ...item, order: index }));
    const data = { data: this.dataSource.data.map(x => ({ _id: x._id, order: x.order })) }
    this.mediaTypeService.editOrders(data)
      .subscribe((res) => {
  ...
  dataSource: MatTableDataSource<MediaType> = new MatTableDataSource<MediaType>();
  getAll() {
  ...
  onSubmit() {
    if (this.form.value) {
      ...
      if (this.modalType === 'add') {
        this.mediaTypeService.create({ ...body, order: this.dataSource.data.length })
          .subscribe(
            (data: any) => {
        ...
        //edit modal service
        this.mediaTypeService.editById(this.form.value._id, body)
          .subscribe(
            (data: MediaType) => {
              var array: MediaType[] = this.dataSource.data;
              var foundIndex = array.findIndex((el: MediaType) => el._id == data._id);
              if (foundIndex > -1) {
  ...
  deleteById(_id, index) {
    this.mediaTypeService.delete(_id)
      .subscribe(
        (res: any) => {
          console.log('res :', res);
