...
export class ListMatiereComponent implements OnInit {
  ...
  onSubmit() {
    if (this.form.value) {
      ...
      var body: Partial<Matiere> = { name: this.form.value.name, description: this.form.value.description };
      if (this.modalType === 'add') {
        this.matiereService.create(body)
          .subscribe(
            (data: any) => {
        ...
        //edit modal service
        this.matiereService.editById(this.form.value._id, body)
          .subscribe(
            (data: Matiere) => {
              var array: Matiere[] = this.dataSource.data;
              var foundIndex = array.findIndex((el: Matiere) => el._id == data._id);
              if (foundIndex > -1) {
      ...
      for (const droppedFile of this.img) {
        if (droppedFile.fileEntry.isFile) {
          ...
          fileEntry.file((file: File) => {
            ...
            this.toasterService.info(DEFAULT_MESSAGES.success.image.loading);
            this.imageService.uploadImage(formData, matiereId, "matiere").subscribe(
              (res) => {
  ...
  deleteById(_id, index) {
    this.matiereService.delete(_id)
      .subscribe(
        (res: any) => {
          console.log('res :', res);
    ...
    for (const droppedFile of this.img) {
      if (droppedFile.fileEntry.isFile) {
        ...
        fileEntry.file((file: File) => {
          ...
          formData.append('document', file, droppedFile.relativePath);
          this.toasterService.info(DEFAULT_MESSAGES.success.image.loading);
          this.imageService.uploadImage(formData, this.form.value._id, "matiere").subscribe(
            (res) => {
